#ifndef __CONFIG
#define __CONFIG



#define NAME "Tower-Defense"
#define ICON "res/image"
#define BACKGROUND_MENU "res/menu.jpg"



#define SIZE_X 28 /* XSize of the game area */
#define SIZE_Y 22 /* YSize of the game area */
#define SCALE 20  /* Difference on size between game and it's graphical representation */

#define SIGHT_TOWER 4 /*Vision of Tower*/

#define SPEED 0.03                       /* Speed of all objects */


#define MAX_MANA 5000    /*Mana gauge*/

#define SIZE_TOWER 0.5 /* Size of player cursor*/
#define BASE_TOWER_X 5 /*Size of player's base X*/
#define BASE_TOWER_Y 5 /*Size of player's base Y*/
#define SIZE_WALL 1     /*Wall thickness*/


#define PV_MONSTER 100
#define SPEED_MONSTER 0.09 /*a voir*/


#endif